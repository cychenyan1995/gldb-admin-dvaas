    ...
    const sidebarOptions = sidebar && (sidebar as IsidebarDetail[]).map((item) => {
      ...
      return (
        <ProviderSidebar
          ...
          title={title}
        >
          <i className={iconClassName} title={title}/>
        </ProviderSidebar>
      )
    })
...
export function mapDispatchToProps (dispatch) {
  return {
    onPageLoad: () => {
      const sidebarSource = [
        { icon: 'icon-dashboard', route: ['vizs', 'dashboard'], permission: 'viz',title: '报告' },
        { icon: 'icon-widget-gallery', route: ['widgets'], permission: 'widget',title: '数据报表' },
        { icon: 'icon-custom-business', route: ['bizlogics', 'bizlogic'], permission: 'view',title: '数据模型' },
        { icon: 'icon-datasource24', route: ['sources'], permission: 'source',title: '数据源' },
        { icon: 'anticon anticon-clock-circle-o', route: ['schedule'], permission: 'schedule',title: '任务调度' }
      ]
