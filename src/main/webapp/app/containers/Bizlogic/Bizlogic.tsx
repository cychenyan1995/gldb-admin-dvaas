...
export class Bizlogic extends React.Component<IBizlogicFormProps, IBizlogicFormState> {
  ...
  private executeSql = () => {
    ...
    this.props.onExecuteSql(sourceIdGeted, sql, (result) => {
      if (result) {
        ...
        columns.forEach((k, index) => {
          tableColumnsC.push({
            title: k.comment === '' ? k.name : k.comment,
            dataIndex: k.name,
            key: index,
            className: `${utilStyles.textAlignLeft}`,
            width: 80
        ...
        this.setState({
          ...
          tableColumns: tableColumnsC,
          tableColumnsC: tableColumnsC,
          tableColumnsE: tableColumnsE,
          switchBtn: true
        })
      }
    })
  ...
  private onDragEnd = (event) => {
    ...
    // 是不是父节点  props.root.state.expandedKeys[0]
    let childrenNode = event.node.props.children;
    let isParent = childrenNode ? true : false;
    // 拖拽的值
    var dragNode = event.node.props.eventKey;
    // 属性值
    let tableprops = this.props.form.getFieldValue("table_props");
    let dragNodePos = event.node.props.pos;

    let { tableArr } = this.state;
    let tableArrTemp = tableArr;
    //只能操作2张表
    if (isParent && tableArr.length === 2) {
      let that = this;
      confirm({
        title: '您是否确认要添加这项内容',
        content: '将清空以前的内容！',
        onOk() {
          tableArrTemp = [];
          //this.setState({parentPos: [], dragProps: [], tableArr: []});
          //保存子节点  选择条件时使用
          let childrenProp = [];
          childrenNode.map(item => childrenProp.push(item.key))

          // 恢复初始化状态
          that.props.form.setFieldsValue({ table_name: '', table_props: '' });
          that.codeMirrorInstanceOfQuerySQL.doc.setValue(``);

          //保存第一张表的信息
          tableArrTemp.push({ tableName: dragNode, parentPos: dragNodePos, childrenProp });
          that.setState({ dragProps: [], tableArr: tableArrTemp });

          let tables = that.oprateNickName(tableArrTemp);
          that.codeMirrorInstanceOfQuerySQL.doc.setValue(`select * from ${tables}`);
          that.props.form.setFieldsValue({ table_name: tables });
        },
        onCancel() {
          return;
        }
      ...
      if (isParent) {
          ...
          childrenNode.map(item => childrenProp.push(item.key))
          //多表查询
          let { parentPos } = this.state;
          // 表名
          if (tableArrTemp.filter(item => item.tableName === dragNode).length === 0) {
            tableArrTemp.push({ tableName: dragNode, parentPos: dragNodePos, childrenProp });
          }
          let tables = this.oprateNickName(tableArrTemp);
          this.props.form.setFieldsValue({ table_name: tables, table_props: '' });
          this.codeMirrorInstanceOfQuerySQL.doc.setValue(`select * from ${tables}`);
          //将父节点的pos保存0-0  子节点的集合置空
          if (!parentPos.includes(dragNodePos)) {
            parentPos.push(dragNodePos);
          }
          this.setState({ parentPos, dragProps: [], tableArr: tableArrTemp });

      } else {
        //得到现有字段属性值
        let { dragProps } = this.state;
        //将新拖拽的字段放入数组中
        //let flag = childrenNodes.filter(item => item.key ===  dragNode);
        //拖拽的子节点是不是当前父节点中的

        let isTables = 0;

        tableArr.map(item => {
          if (item.parentPos === dragNodePos.substr(0, item.parentPos.length)) {
            //a.id
            let nickName = '';
            let tmp = item.tableName.split('_')
            tmp.length > 1 ? tmp.map(item => nickName += item.substring(0, 1)) : nickName = item.tableName;
            tmp.length > 1 ? nickName = nickName + tmp[tmp.length - 1] : '';
            dragProps.push(`${nickName}.${dragNode}`);
            // 数组去重
            dragProps = Array.from(new Set(dragProps));
            this.setState({ dragProps: dragProps });
            let dragPropsSql = dragProps.join(',');
            // 给字段部分设值
            this.props.form.setFieldsValue({ table_props: dragPropsSql });
            let tableName = this.props.form.getFieldValue("table_name");
            this.codeMirrorInstanceOfQuerySQL.doc.setValue(`select ${dragPropsSql} from ${tableName}`);
          } else {
            isTables++;
            return;
          }
        ...
        //给出提示框
        isTables === 2 && message.error('请选择目标表名中的字段');
      }
      // flag
  ...
  private oprateNickName(tableArrTemp) {
    return tableArrTemp.length > 0 ? [...(tableArrTemp.map(item => {
      let nickName = '';
      let tmp = item.tableName.split('_');
      tmp.length > 1 ? tmp.map(item => nickName += item.substring(0, 1)) : item.tableName;
      tmp.length > 1 ? nickName = nickName + tmp[tmp.length - 1] : '';
      return `${item.tableName} ${nickName}`;
    }))].join(',') : '';
  }

  private onDragEnter(opt) {
  ...
  //switch开关
  private switchChange = (isChecked) => {
    //英
    if (!isChecked) {
      this.setState({ tableColumns: this.state.tableColumnsE });
    } else {
      this.setState({ tableColumns: this.state.tableColumnsC });
    }
  }

  // modal
  private factorModal = () => {
    this.setState({ factorVisible: true });
  }

  private factorHideForm = () => {
    this.setState({ factorVisible: false });
    this.factorForm.props.form.resetFields();
  }

  private handleOk = () => {
    this.factorForm.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        const { mainTable, sideTable } = values;
        let sql = this.codeMirrorInstanceOfQuerySQL.doc.getValue();
        let index = sql.indexOf('where');
        let sqlW = '';
        index === -1 ? sqlW = sql.slice() : sqlW = sql.slice(0, index - 1);
        this.codeMirrorInstanceOfQuerySQL.doc.setValue(`${sqlW} where ${mainTable}=${sideTable}`)
        this.setState({ factorVisible: false });
        this.factorForm.props.form.resetFields();
  ...
  private factorForm: AntdFormType = null;
  ...
  public render() {
    ...
    const operations = (
      <div>
        <Switch
          className={`${this.state.switchBtn && isFold ? styles.switchshow : styles.switchHide}`}
      ...
      <div className={styles.bizlogic}>
        ...
        <Form className={styles.formView}>
          ...
          <Row className={styles.formRight}>
            ...
            <Col span={24} className={styles.tableSelect}>
              <FormItem label="表名">
            ...
            <Col span={24} className={`small-item-margin ${styles.declareSelect}`}>
              ...
              <Button type="primary" disabled={tableArr.length < 2 ? true : false} className={styles.factorBtn} onClick={this.factorModal}>
                条件
              </Button>
              <Modal
                title={`添加对应关系`}
                wrapClassName="ant-modal-small"
                visible={factorVisible}
                onCancel={this.factorHideForm}
                onOk={this.handleOk}
              >
                <FactorForm tableArr={tableArr} wrappedComponentRef={(ref) => this.factorForm = ref} />
              </Modal>
            </Col>
